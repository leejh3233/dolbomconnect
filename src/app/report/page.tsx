        ...
        <main className="p-4 max-w-xl mx-auto bg-white shadow-lg rounded-xl my-8">
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    {/* 추천인이 "없음"이 아니고 아파트 목록이 있으면 select 드롭다운 사용 */}
                    {isAptEnabled && form.추천인 !== "없음" && aptList.length > 0 ? (
                        ...
                        <input
                            ...
                            value={form.아파트명}
                            onChange={(e) => {
                                // 추천인이 선택되어 있고 "없음"이 아닌 경우, 목록에 없는 값 입력 방지
                                if (form.추천인 && form.추천인 !== "없음" && form.추천인.trim().length >= 2 && aptList.length > 0) {
                                    const isValidApt = aptList.some((apt: any) => apt.aptName === e.target.value);
                                    if (!isValidApt && e.target.value !== "") {
                                        alert("항목에서 선택해주세요. 추천인으로 예약된 아파트 목록에서만 선택 가능합니다.");
                                        return;
                                    }
                                }
                                handleChange(e);
                            ...
                            placeholder={isAptEnabled ? (form.추천인 === "없음" ? "아파트명을 입력하세요" : "아파트 목록 로딩 중...") : "추천인을 먼저 입력하세요"}
                    ...
                    {isAptEnabled && form.추천인 !== "없음" && aptList.length > 0 && (
                        <p className="text-[10px] text-blue-500 mt-1 ml-1">* 예약된 아파트 목록에서 선택해주세요.</p>
                    )}
                </div>
